...
def order_by_total_payout(filename):
	...
	"""
	campaigns = get_campaigns(filename)
	campaigns.sort(key=lambda x: float(x['payout'].replace(',','.')) * int(x['installs']), reverse=True)
...
def order_by_cr(filename):
	...
	"""
	campaigns = get_campaigns(filename)
	campaigns.sort(key=lambda x: float(x['impressions']) / float(x['installs']), reverse=False)
